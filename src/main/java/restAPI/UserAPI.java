    ...
    public class UserAPI {
        ...
        // ✅ GET user by ID
@GET
@Path("userId/{id}")  // Accepts user ID as a path parameter
@Produces(MediaType.APPLICATION_JSON)
public Response getUserById(@PathParam("id") int id) {
    Users user = userService.getUserById(id);  // Ensure this method correctly retrieves a single user
    ...
    return Response.status(Response.Status.NOT_FOUND)
                   .entity("{\"message\": \"User not found with ID: " + id + "\"}")
                   .build();
}
@GET
@Path("/email/{email}")
