...
public class GalleryAPI {
 ...
 public static class GalleryDTO {
    ...
    // ✅ Constructor with only Gallery (for older methods)
    public GalleryDTO(Gallery g) {
        ...
        this.photographerName = null; // or fetch manually if needed
    }

    // ✅ Constructor with Gallery and photographerName (for new method)
    public GalleryDTO(Gallery g, String photographerName) {
        this.imageID = g.getImageID();
        this.imageURL = g.getImageURL();
        this.photographerID = g.getPhotographerID().getPhotographerID();
...
@GET
@Path("/with-photographer/{photographerID}")
public List<GalleryDTO> getGalleryImagesWithPhotographerName(@PathParam("photographerID") int photographerID) {
    // Use actual DB/service call to fetch photographer
    Photographers photographer = galleryService.getPhotographerById(photographerID);
    if (photographer == null) {
        return List.of(); // or throw 404
    }

    String fullName = photographer.getFullName();

    List<Gallery> galleryImages = galleryService.getGalleryImagesByPhotographer(photographer);

    return galleryImages.stream()
            .map(g -> new GalleryDTO(g, fullName))
            .collect(Collectors.toList());
}

@GET
@Path("/photographer/ids/{ids}")
public List<GalleryDTO> getGalleryImagesByPhotographerIDs(@PathParam("ids") String ids) {
    List<GalleryDTO> allImages = new java.util.ArrayList<>();

    String[] idArray = ids.split(",");
    for (String idStr : idArray) {
        try {
            int photographerID = Integer.parseInt(idStr.trim());
            ...
            List<GalleryDTO> dtoList = galleryImages.stream()
                    .map(g -> new GalleryDTO(g, fullName))
                    .collect(Collectors.toList());

            allImages.addAll(dtoList);
